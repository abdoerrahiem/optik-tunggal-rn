...
function ProductDetailSerupa() {
  // Hooks
  const navnav = useNavigation()
  const navigation = useAppNavigation()
  const route = useRoute<RouteProp<PublicHomeStackParamList, 'ProductDetailSerupa'>>()
  ...
  const defaultColor = route.params.product?.jeniswarna === 'clear' ? 'Clear' : warnaDefault?.nm_warna

  const handleCartAdd = async (tipe: string) => {
    console.log('ISINYA : ', fields.color)
    if (tipe == 'contactlens') {
      if(fields.color !== '' && fields.color2 !== ''){
        if(fields.spheries == '' || fields.basecurve == '' || fields.jumlah == 0){
          ToastAndroid.show(`${''}Pilihan mata kiri belum dilengkapi!`, ToastAndroid.SHORT)
        }else if(fields.color2 == '' || fields.spheries2 == '' || fields.basecurve2 == '' || fields.jumlah2 == 0){
          ToastAndroid.show(`${''}Pilihan mata kanan belum dilengkapi!`, ToastAndroid.SHORT)
        }else{
          toCartLens()
        }
        ...
        if(fields.spheries2 == '' || fields.basecurve2 == '' || fields.jumlah2 == 0){
          ToastAndroid.show(`${''}Pilihan mata kanan belum dilengkapi!`, ToastAndroid.SHORT)
        }else if(fields.color == '' || fields.spheries == '' || fields.basecurve == '' || fields.jumlah == 0){
          toCartLens()  
        }
      }else{
        ToastAndroid.show(`${''}Pilihan belum dilengkapi!`, ToastAndroid.SHORT)
      }
...
export default ProductDetailSerupa
